...
struct BRDFData // 声明需要的变量
{
    half3 albedo;// 基本颜色
    half3 diffuse;// 漫反射
    half3 specular;// 镜面反射
    half reflectivity;// 反射率
    half perceptualRoughness;// 感知粗糙度
    half roughness;// 粗糙度
    half roughness2;// 粗糙度2
    half grazingTerm;// 在掠射角时的反射行为的参数

    // We save some light invariant BRDF terms so we don't have to recompute 我们保存了一些与光源无关的BRDF（双向反射分布函数）项，这样我们就不需要在光照循环中重复计算它们。
    // them in the light loop. Take a look at DirectBRDF function for detailed explaination.
    half normalizationTerm;     // roughness * 4.0 + 2.0
    half roughness2MinusOne;    // roughness^2 - 1.0 将计算结果限制在(-1,0)之间
};
